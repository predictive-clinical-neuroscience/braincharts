...
idp_ids = ['rh_MeanThickness_thickness']
idp_ids = ['WM-hypointensities']
idp_ids = ['Right-Lateral-Ventricle']
#idp_ids = ['Right-Hippocampus']

idp_ids = df_te.columns.to_list()[15:90]
#idp_ids = df_te.columns.to_list()[90:91]
#idp_ids = df_te.columns.to_list()[208:222]
idp_ids = df_te.columns.to_list()[15:207] + df_te.columns.to_list()[208:222]
...
for sid, site in enumerate(cols_site):
    ...
    #np.savetxt(os.path.join(data_dir,'cov_dummy_' + site +'.txt'), X_dummy)
    # without intercept
    #Xz_dummy = np.concatenate((X_dummy, np.array([B(i) for i in X_dummy[:,0]])), axis=1)
...
for idp in idp_ids: 
    ...
    plt.title(idp)
    plt.xlim((0,90))
    plt.savefig(os.path.join(idp_dir, out_name, 'centiles'),  bbox_inches='tight')
    plt.show()
